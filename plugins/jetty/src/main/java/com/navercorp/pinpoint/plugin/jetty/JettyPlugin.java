...
import com.navercorp.pinpoint.bootstrap.instrument.transformer.TransformUtils;
...
public class JettyPlugin implements ProfilerPlugin, TransformTemplateAware {
  ...
  private void addServerInterceptor(final JettyConfiguration config) {
    transformTemplate.transform("org.eclipse.jetty.server.Server", new TransformCallback() {
      ...
      public byte[] doInTransform(Instrumentor instrumentor, ClassLoader classLoader,
        ...
        InstrumentMethod handleMethodEditorBuilder =
            target.getDeclaredMethod("handle", "org.eclipse.jetty.server.HttpChannel");
        if (handleMethodEditorBuilder != null) {
          handleMethodEditorBuilder.addInterceptor(
              "com.navercorp.pinpoint.plugin.jetty.interceptor.ServerHandleInterceptor",
              va(config.getJettyExcludeUrlFilter()));
          return target.toBytecode();
        }

        InstrumentMethod jetty8HandleMethodEditorBuilder =
            target.getDeclaredMethod("handle", "org.eclipse.jetty.server.AbstractHttpConnection");
        if (jetty8HandleMethodEditorBuilder != null) {
          jetty8HandleMethodEditorBuilder.addInterceptor(
  ...
  private void addRequestEditor() {
    final TransformCallback transformCallback =
        TransformUtils.addField("com.navercorp.pinpoint.plugin.jetty.interceptor.TraceAccessor");
