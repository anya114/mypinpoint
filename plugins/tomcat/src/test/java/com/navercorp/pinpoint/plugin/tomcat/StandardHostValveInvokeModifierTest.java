...
public class StandardHostValveInvokeModifierTest extends BasePinpointTest {
  ...
  private void initMockResponse() {
    doAnswer(new Answer<String>() {
      ...
      public String answer(InvocationOnMock invocation) throws Throwable {
        headers.put((String) invocation.getArguments()[0], invocation.getArguments()[1]);
        return (String) invocation.getArguments()[1];
      }
    }).when(mockResponse).addHeader(Mockito.anyString(), Mockito.anyString());
    doAnswer(new Answer<String>() {
      @Override
      public String answer(InvocationOnMock invocation) throws Throwable {
        return (String) headers.get(invocation.getArguments()[0]);
      }

    }).when(mockResponse).getHeader(Mockito.anyString());
  }
  ...
  public void invokeShouldContinueTracingFromRequest() throws Exception {
    ...
    final String sourceTransactionId = TransactionIdUtils.formatString(sourceAgentId,
        sourceAgentStartTime, sourceTransactionSequence);
    ...
    when(mockRequest.getHeader(Header.HTTP_PARENT_SPAN_ID.toString()))
        .thenReturn(String.valueOf(sourceParentId));
    // When
    host.invoke(mockRequest, mockResponse);
    // Then
    final List<SpanBo> rootSpans = getCurrentRootSpans();
  ...
  public void wrapResponseTest() throws Exception {
    host.invoke(mockRequest, mockResponse);
    assertNotNull(mockResponse.getHeader(Header.HTTP_TRACE_ID.toString()));
  }
}
