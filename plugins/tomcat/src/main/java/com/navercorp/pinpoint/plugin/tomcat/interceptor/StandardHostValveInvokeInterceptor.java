...
public class StandardHostValveInvokeInterceptor implements AroundInterceptor {
  public static final ServletSyncMethodDescriptor SERVLET_SYNCHRONOUS_API_TAG =
      new ServletSyncMethodDescriptor();
  public static final ServletAsyncMethodDescriptor SERVLET_ASYNCHRONOUS_API_TAG =
      new ServletAsyncMethodDescriptor();
  ...
  public StandardHostValveInvokeInterceptor(TraceContext traceContext, MethodDescriptor descriptor,
    ...
    if (proxyIpHeader == null || proxyIpHeader.isEmpty()) {
      ...
      this.remoteAddressResolver =
          new RealIpHeaderResolver<HttpServletRequest>(proxyIpHeader, tomcatRealIpEmptyValue);
    }
  ...
  public void before(Object target, Object[] args) {
    ...
    try {
      ...
      wrapResponse(trace, (Response)args[1]);
      if (trace == null) {
        return;
      }
  ...
  private void wrapResponse(Trace trace, Response response) {
    if(trace != null) {
      response.addHeader(Header.HTTP_TRACE_ID.toString(), trace.getTraceId().getTransactionId());
    }
  }
  ...
  private Trace createTrace(Object target, Object[] args) {
    ...
    if (!sampling) {
      // Even if this transaction is not a sampling target, we have to create Trace object to mark
      // 'not sampling'.
      // For example, if this transaction invokes rpc call, we can add parameter to tell remote node
      // 'don't sample this transaction'
    ...
    if (traceId != null) {
      // TODO Maybe we should decide to trace or not even if the sampling flag is true to prevent
      // too many requests are traced.
      ...
      if (trace.canSampled()) {
        ...
        if (isDebug) {
          logger.debug("TraceID exist. continue trace. traceId:{}, requestUrl:{}, remoteAddr:{}",
  ...
  private void recordParentInfo(SpanRecorder recorder, HttpServletRequest request) {
    String parentApplicationName =
        request.getHeader(Header.HTTP_PARENT_APPLICATION_NAME.toString());
    if (parentApplicationName != null) {
      ...
      if (host != null) {
        ...
        recorder
            .recordAcceptorHost(NetworkUtils.getHostFromURL(request.getRequestURL().toString()));
      }
      final String type = request.getHeader(Header.HTTP_PARENT_APPLICATION_TYPE.toString());
      final short parentApplicationType =
          NumberUtils.parseShort(type, ServiceType.UNDEFINED.getCode());
   ...
   *
   * @param request
   * @return TraceId when it is possible to get a transactionId from Http header. if not possible
   *         return null
   */
  private TraceId populateTraceIdFromRequest(HttpServletRequest request) {
    ...
    if (transactionId != null) {
      ...
      long parentSpanID = NumberUtils
          .parseLong(request.getHeader(Header.HTTP_PARENT_SPAN_ID.toString()), SpanId.NULL);
      long spanID =
          NumberUtils.parseLong(request.getHeader(Header.HTTP_SPAN_ID.toString()), SpanId.NULL);
      short flags =
          NumberUtils.parseShort(request.getHeader(Header.HTTP_FLAGS.toString()), (short) 0);
  ...
  private void deleteTrace(Trace trace, Object target, Object[] args, Object result,
      Throwable throwable) {
    trace.traceBlockEnd();
