...
public class MySqlPlugin implements ProfilerPlugin, TransformTemplateAware {
  ...
  public void setup(ProfilerPluginSetupContext context) {
    ...
    // Driver returns not com.mysql.jdbc.Connection but com.mysql.jdbc.JDBC4Connection which extends
    // com.mysql.jdbc.ConnectionImpl from 5.1.x
    ...
    transformTemplate.transform("com.mysql.jdbc.NonRegisteringDriver", new TransformCallback() {
      ...
      public byte[] doInTransform(Instrumentor instrumentor, ClassLoader loader, String className,
        ...
        target.addScopedInterceptor(
            ...
            va(new MySqlJdbcUrlParser(), false), MySqlConstants.MYSQL_SCOPE,
    ...
    transformTemplate.transform("com.mysql.jdbc.PreparedStatement", new TransformCallback() {
      ...
      public byte[] doInTransform(Instrumentor instrumentor, ClassLoader loader, String className,
        ...
        target.addScopedInterceptor(
            ...
            va(maxBindValueSize), MySqlConstants.MYSQL_SCOPE);
        ...
        if (config.isTraceSqlBindValue()) {
          ...
          target.addScopedInterceptor(excludes,
              ...
              MySqlConstants.MYSQL_SCOPE, ExecutionPolicy.BOUNDARY);
        }
  ...
  private void addJDBC4PreparedStatementTransformer(final MySqlConfig config) {
    transformTemplate.transform("com.mysql.jdbc.JDBC4PreparedStatement", new TransformCallback() {
      ...
      public byte[] doInTransform(Instrumentor instrumentor, ClassLoader loader, String className,
        ...
        if (config.isTraceSqlBindValue()) {
          final PreparedStatementBindingMethodFilter includes =
              PreparedStatementBindingMethodFilter.includes("setRowId", "setNClob", "setSQLXML");
          target.addScopedInterceptor(includes,
              "com.navercorp.pinpoint.bootstrap.plugin.jdbc.interceptor.PreparedStatementBindVariableInterceptor",
              MySqlConstants.MYSQL_SCOPE, ExecutionPolicy.BOUNDARY);
        }
