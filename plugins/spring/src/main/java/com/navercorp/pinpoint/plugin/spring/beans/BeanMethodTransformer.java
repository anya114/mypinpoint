...
public class BeanMethodTransformer implements TransformCallback {
    ...
    try {
      ...
      for (InstrumentMethod method : methodList) {
        if (logger.isDebugEnabled()) {
          logger.debug("### c={}, m={}, params={}", className, method.getName(),
  ...
  private void addInterceptor(InstrumentMethod targetMethod) throws InstrumentException {
    ...
    synchronized (lock) {
      ...
      id = targetMethod.addInterceptor(
          "com.navercorp.pinpoint.plugin.spring.beans.interceptor.BeanMethodInterceptor",
          va(markError));
