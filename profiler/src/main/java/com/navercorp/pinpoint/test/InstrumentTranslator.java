...
public class InstrumentTranslator implements Translator {
  ...
  private final ConcurrentMap<String, MatchableClassFileTransformer> transformerMap =
      new ConcurrentHashMap<String, MatchableClassFileTransformer>();
  ...
  private void addTransformer0(MatchableClassFileTransformer transformer, String className) {
    ...
    if (old != null) {
      throw new IllegalStateException(
          "Modifier already exist new:" + transformer.getClass() + " old:" + old.getMatcher());
    }
  }

  @Override public void start(ClassPool pool) throws NotFoundException, CannotCompileException {
    //        this.pool = pool;
  }

  @Override public void onLoad(ClassPool pool, String javaClassName)
  ...
  private void onLoadTestModifier(ClassPool pool, String jvmClassName)
      throws NotFoundException, CannotCompileException {
    ...
    logger
        .info("Modify jvmClassName:{},  modifier{}, loader:{}", jvmClassName, transformer, loader);
  ...
  private void makeClass(ClassPool pool, byte[] transform, String jvmClassName) {
    try {
      ...
      throw new RuntimeException(
          "Class make fail. jvmClass:" + jvmClassName + " Caused by:" + ex.getMessage(), ex);
    }
  }
}
