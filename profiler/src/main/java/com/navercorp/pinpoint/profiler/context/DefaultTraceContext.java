...
public class DefaultTraceContext implements TraceContext {
  ...
  private final TransactionCounter transactionCounter =
      new DefaultTransactionCounter(this.idGenerator);

  // for test
  public DefaultTraceContext(final AgentInformation agentInformation) {
    this(LRUCache.DEFAULT_CACHE_SIZE, agentInformation, new LogStorageFactory(), new TrueSampler(),
        new DefaultServerMetaDataHolder(RuntimeMXBeanUtils.getVmArgs()), TRACE_ACTIVE_THREAD);
  }

  public DefaultTraceContext(final int sqlCacheSize, final AgentInformation agentInformation,
      StorageFactory storageFactory, Sampler sampler, ServerMetaDataHolder serverMetaDataHolder,
  ...
  private TraceFactory createTraceFactory(StorageFactory storageFactory, Sampler sampler,
      boolean recordActiveThread) {
    // TODO extract chain TraceFactory??
    final TraceFactory threadLocalTraceFactory =
        new ThreadLocalTraceFactory(this, storageFactory, sampler, this.idGenerator);
    if (recordActiveThread) {
      ActiveTraceFactory activeTraceFactory =
          (ActiveTraceFactory) ActiveTraceFactory.wrap(threadLocalTraceFactory);
  ...
  @Override
  public TraceId createTraceId(final String transactionId, final long parentSpanID,
      final long spanID, final short flags) {
