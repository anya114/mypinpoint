...
public class AnnotatedInterceptorFactory implements InterceptorFactory {
  ...
  public Interceptor getInterceptor(ClassLoader classLoader, String interceptorClassName,
      Object[] providedArguments, InterceptorScope scope, ExecutionPolicy policy,
      InstrumentClass target, InstrumentMethod targetMethod) {
    Class<? extends Interceptor> interceptorType =
        pluginContext.injectClass(classLoader, interceptorClassName);
    ...
    ObjectFactory objectFactory =
        ObjectFactory.byConstructor(interceptorClassName, providedArguments);
    InterceptorArgumentProvider interceptorArgumentProvider = new InterceptorArgumentProvider(
        pluginContext.getTraceContext(), scope, target, targetMethod);

    Interceptor interceptor =
        (Interceptor) factory.createInstance(objectFactory, interceptorArgumentProvider);

    if (scope != null) {
      interceptor =
          wrapByScope(interceptor, scope, policy == null ? ExecutionPolicy.BOUNDARY : policy);
    }

    return interceptor;
  }

  private Interceptor wrapByScope(Interceptor interceptor, InterceptorScope scope,
    ...
    if (interceptor instanceof AroundInterceptor) {
      ...
      return new ScopedStaticAroundInterceptor((StaticAroundInterceptor) interceptor, scope,
          policy);
      ...
      return new ScopedApiIdAwareAroundInterceptor((ApiIdAwareAroundInterceptor) interceptor, scope,
          policy);
    }
