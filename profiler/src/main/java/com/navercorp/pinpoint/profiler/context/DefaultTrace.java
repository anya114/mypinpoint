...
public final class DefaultTrace implements Trace {
  ...
  public DefaultTrace(final TraceContext traceContext, long transactionId, boolean sampling) {
    ...
    this.traceId = new DefaultTraceId(traceContext.getAgentId(), traceContext.getAgentStartTime(),
        transactionId);
  ...
  public DefaultTrace(TraceContext traceContext, TraceId continueTraceId, long transactionId,
      boolean sampling) {
    if (traceContext == null) {
      throw new NullPointerException("traceContext must not be null");
    }
  ...
  @Override public SpanEventRecorder traceBlockBegin() {
    return traceBlockBegin(DEFAULT_STACKID);
  }

  @Override public SpanEventRecorder traceBlockBegin(final int stackId) {
  ...
  @Override public void traceBlockEnd() {
    traceBlockEnd(DEFAULT_STACKID);
  }

  @Override public void traceBlockEnd(int stackId) {
    if (this.closed) {
      if (isWarn) {
    ...
    if (spanEvent.getStackId() != stackId) {
      ...
      if (isWarn) {
        PinpointException exception = new PinpointException(
            "not matched stack id. expected=" + stackId + ", current=" + spanEvent.getStackId());
        logger.warn("[DefaultTrace] Corrupted call stack found.", exception);
      }
    }
  ...
  @Override public void close() {
    ...
    // If the stack is not handled properly, NullPointerException will be thrown AFTER this. Is it
    // OK?
  ...
  @Override public Thread getBindThread() {
    return bindThread;
  }
...
//    if(spanEvent.getEndElapsed() != 0)
  ...
  @Override public boolean isRootStack() {
    return callStack.empty();
  }

  @Override public AsyncTraceId getAsyncTraceId() {
    return new DefaultAsyncTraceId(traceId, traceContext.getAsyncId(),
        spanRecorder.getSpan().getStartTime());
  }

  @Override public SpanRecorder getSpanRecorder() {
    return spanRecorder;
  }

  @Override public SpanEventRecorder currentSpanEventRecorder() {
  ...
  @Override public int getCallStackFrameId() {
  ...
  @Override public TraceType getTraceType() {
