...
public class SpanStreamUdpSender extends AbstractDataSender {
  ...
  public SpanStreamUdpSender(String host, int port, String threadName, int queueSize, int timeout,
      int sendBufferSize) {
    this(host, port, threadName, queueSize, SOCKET_TIMEOUT, SEND_BUFFER_SIZE, DEFAULT_BUFFER_SIZE);
  }

  public SpanStreamUdpSender(String host, int port, String threadName, int queueSize, int timeout,
      int sendBufferSize, int dataBufferSize) {
    if (host == null) {
    ...
    HeaderTBaseSerializerPoolFactory headerTBaseSerializerPoolFactory =
        new HeaderTBaseSerializerPoolFactory(false, dataBufferSize, true);
    this.serializerPool =
        new ObjectPool<HeaderTBaseSerializer>(headerTBaseSerializerPoolFactory, 16);

    this.spanStreamSendDataSerializer = new SpanStreamSendDataSerializer();

    this.spanStreamSendDataFactory =
        new SpanStreamSendDataFactory(dataBufferSize, 16, serializerPool);

    this.standbySpanStreamDataSendWorker =
        new StandbySpanStreamDataSendWorker(new FlushHandler(), new StandbySpanStreamDataStorage());
    ...
    try {
      ...
      if (logger.isWarnEnabled()) {
        ...
        if (sendBufferSize != checkSendBufferSize) {
          logger.warn("DatagramChannel.setSendBufferSize() error. {}!={}", sendBufferSize,
              checkSendBufferSize);
        }
      }
  ...
  private void handleSpan(Span span) {
    ...
    CompositeSpanStreamData compositeSpanStreamData =
        spanStreamSendDataSerializer.serializeSpanStream(serializer, span);
  ...
  private void handleSpanChunk(SpanChunk spanChunk) {
    ...
    HeaderTBaseSerializer serializer = serializerPool.getObject();
    CompositeSpanStreamData compositeSpanStreamData =
        spanStreamSendDataSerializer.serializeSpanChunkStream(serializer, spanChunk);
    if (compositeSpanStreamData == null) {
      serializerPool.returnObject(serializer);
      return;
    }
  ...
  private void doAddAndFlush(CompositeSpanStreamData compositeSpanStreamData,
      HeaderTBaseSerializer serializer) {
    logger.debug("CompositeSpanStreamData {}.", compositeSpanStreamData);

    SpanStreamSendData currentSpanStreamSendData =
        standbySpanStreamDataSendWorker.getStandbySpanStreamSendData();
    ...
    try {
      if (!currentSpanStreamSendData.addBuffer(compositeSpanStreamData.getByteBuffer())) {
        SendDataPlaner sendDataPlaner =
            new SpanChunkStreamSendDataPlaner(compositeSpanStreamData, spanStreamSendDataFactory);

        Iterator<SpanStreamSendData> sendDataIterator =
            sendDataPlaner.getSendDataIterator(currentSpanStreamSendData, serializer);
