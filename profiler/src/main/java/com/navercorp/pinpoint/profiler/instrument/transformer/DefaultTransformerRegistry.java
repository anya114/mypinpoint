...
public class DefaultTransformerRegistry implements TransformerRegistry {
  ...
  // No concurrent issue because only one thread put entries to the map and get operations are
  // started AFTER the map is completely build.
  // Set the map size big intentionally to keep hash collision low.
  private final Map<String, ClassFileTransformer> registry =
      new HashMap<String, ClassFileTransformer>(512);
...
//    if (old != null) {
//      logger.debug("Transformer already exist. className: {} new : {} old : {}", jvmClassName,
//          transformer.getClass(), old.getClass());
//    }
    //without the throw, it may cause deadloop
    if (old != null) {
      throw new IllegalStateException("Transformer already exist. className:" + jvmClassName
          + " new:" + transformer.getClass() + " old:" + old.getClass());
    }
  }
}
